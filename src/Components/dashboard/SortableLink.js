...
export default function SortableLink({
  ...
  // Validation Logic: Neutral if empty, Error if missing 'http'
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`relative p-3 sm:p-5 rounded-2xl border flex gap-3 items-start sm:items-center shadow-sm group touch-none transition-colors ${
        hasUrlError
      ...
      role="listitem"
    >
      {/* The Handle: Aligned to top on mobile for better ergonomics with larger inputs */}
      <button
        type="button"
        {...attributes}
        {...listeners}
        aria-label="Drag to reorder link"
        className="cursor-grab active:cursor-grabbing p-2 mt-1 sm:mt-0 text-slate-300 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 shrink-0"
      >
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 shrink-0 mt-1 sm:mt-0">
        {/* Analytics: Hidden on mobile to save space, visible on tablet+ */}
        <div
